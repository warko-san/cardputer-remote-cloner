    ...
    case Process::COPY_MAIN_CONTROLS:
        if (prevData.command == currentData.command && prevData.address == currentData.address && prevData.protocol == currentData.protocol)
        {
            if (saveConfirmations == 1)
            {
                ...
                screenProvider.printCommandData(currentData.address, currentData.command, false);
                saveConfirmations++;
            }
        }
        else
        {
            ...
            screenProvider.printCommandData(currentData.address, currentData.command, true);
            saveConfirmations++;
        }
        break;
    case Process::COPY_NUMBERS:
